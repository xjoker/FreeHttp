...
{
    public partial class RuleInfoWindow : CBalloonBase
    {
        private bool isLoadRuleComplete;
        private Rectangle lastListViewItemRectangle;
        private Point nowLocation = Point.Empty;
        ...
        private void LoadRuleInfo(IFiddlerHttpTamper ruleInfo)
        {
            Action<FiddlerRequestChange> GetFiddlerRequestChangeAddition = nowFiddlerRequestChange =>
            {
                if ((nowFiddlerRequestChange.ParameterPickList != null &&
                     nowFiddlerRequestChange.ParameterPickList.Count > 0) || nowFiddlerRequestChange.IsHasParameter)
                {
                    ...
                    if (nowFiddlerRequestChange.ParameterPickList != null &&
                        nowFiddlerRequestChange.ParameterPickList.Count > 0)
            ...
            Action<FiddlerResponseChange> GetFiddlerResponseChangeAddition = nowFiddlerResponseChange =>
            {
                if ((nowFiddlerResponseChange.ParameterPickList != null &&
                     nowFiddlerResponseChange.ParameterPickList.Count > 0) || nowFiddlerResponseChange.IsHasParameter ||
                    nowFiddlerResponseChange.ResponseLatency > 0)
                {
                    rtb_ruleInfo.AddRtbStr("Addition ", Color.Red, true, new Font(FontFamily.GenericMonospace, 14));
            ...
            if (ruleInfo.HttpFilter.HeadMatch != null && ruleInfo.HttpFilter.HeadMatch.HeadsFilter.Count > 0)
                foreach (var tempHeaderFilter in ruleInfo.HttpFilter.HeadMatch.HeadsFilter)
                {
                    ...
                    rtb_ruleInfo.AppendText(string.Format("{0} [contain] {1}", tempHeaderFilter.Key,
                        tempHeaderFilter.Value));
                    rtb_ruleInfo.AppendText("\r\n");
                }
            ...
            switch (ruleInfo.TamperProtocol)
            {
                case TamperProtocalType.Http:
                    ...
                    {
                        lb_ruleId.Text = string.Format("Http Request Tamper Rule {0}",
                            InnerListViewItem.SubItems[0].Text);

                        var nowFiddlerRequestChange = ruleInfo as FiddlerRequestChange;
                        ...
                        else
                        {
                            ...
                            if (nowFiddlerRequestChange.HeadDelList != null &&
                                nowFiddlerRequestChange.HeadDelList.Count > 0)
                            ...
                            if (nowFiddlerRequestChange.HeadAddList != null &&
                                nowFiddlerRequestChange.HeadAddList.Count > 0)
                    ...
                    {
                        lb_ruleId.Text = string.Format("Http Response Tamper Rule {0}",
                            InnerListViewItem.SubItems[0].Text);

                        var nowFiddlerResponseChange = ruleInfo as FiddlerResponseChange;
                        ...
                        {
                            if (nowFiddlerResponseChange.HeadDelList != null &&
                                nowFiddlerResponseChange.HeadDelList.Count > 0)
                            ...
                            if (nowFiddlerResponseChange.HeadAddList != null &&
                                nowFiddlerResponseChange.HeadAddList.Count > 0)
                ...
                case TamperProtocalType.WebSocket:
                    if (ruleInfo is FiddlerRequestChange)
                    {
                        lb_ruleId.Text = string.Format("Websocket Send Tamper Rule {0}",
                            InnerListViewItem.SubItems[0].Text);
                    ...
                    {
                        lb_ruleId.Text = string.Format("Websocket Receive Tamper Rule {0}",
                            InnerListViewItem.SubItems[0].Text);
        ...
        private void MyCBalloon_Load(object sender, EventArgs e)
        {
            ...
            {
                var nowRule = InnerListViewItem.Tag as IFiddlerHttpTamper;
        ...
        private void Timer_Tick(object sender, EventArgs e)
        {
            ...
            {
                lastListViewItemRectangle = InnerListViewItem.Bounds;
                var mainForm = Owner.Owner;
                var tempFinalY = InnerListViewItem.Bounds.Y;
                if (tempFinalY < 0)
                    tempFinalY = 0;
                else if (tempFinalY > InnerListViewItem.ListView.Height) tempFinalY = InnerListViewItem.ListView.Height;
                var myPosition = new Point(InnerListViewItem.Bounds.X, tempFinalY);
                ...
                UpdateBalloonPosition(myPosition);
            }
        }
        ...
        private void rtb_ruleInfo_ContentsResized(object sender, ContentsResizedEventArgs e)
        {
            ...
            var lineheight = richTextBox.Font.Height;
            var nHeight = lineheight;
            if (e.NewRectangle.Height < lineheight)
                nHeight = lineheight;
            else
                nHeight = e.NewRectangle.Height + lineheight;

            richTextBox.Height = nHeight > Screen.PrimaryScreen.Bounds.Height / 2
                ? Screen.PrimaryScreen.Bounds.Height / 2
                : nHeight;

            Height = richTextBox.Height + 95;
            reSizeMe();
        }
    }
}