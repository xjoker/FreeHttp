...
namespace CBalloon
{
    ...
    /// </summary>
    public class CBalloonBase : Form
    {
        ...
        private CBalloonBase m_fmShadow;
        ...
        public void reSizeMe()
        {
            ...
            m_path.AddLines(new[] { pA, pB, pC });
            m_path.CloseFigure();
            // generate balloon tail inner path
            var iPath = new GraphicsPath();
            iPath.AddLines(new[] { pAi, pBi, pCi });
            iPath.CloseFigure();

            #region generate the outer frame (m_rOuterFrame) which becomes the shape of the balloon form

            var pSize = new Size(nW - (nInner + nInner), nH - (nOuter + nOuter));
            var pLoc = new Point(nInner, nOuter);
            var aRect = new Rectangle(pLoc, pSize);
            ...
            var eg = Graphics.FromHwnd(Handle);
            // calculate the bounding rectangle for this region (within the context of the form itself) and the centre point for
            // the rectangle
            m_rtBalloonF = Region.GetBounds(eg);
            m_ptCenter = new Point((int)(m_rtBalloonF.Width / 2 + m_rtBalloonF.X),
                (int)(m_rtBalloonF.Height / 2 + m_rtBalloonF.Y));
            eg.Dispose();

            #region always generate the outline region (m_rInnerFrame), even if we are not going to display it immediately

            var myRect = new Rectangle(C_QUOTEH + 6, C_QUOTEH + 4, nW - (2 * C_QUOTEH + 12), nH - (2 * C_QUOTEH + 8));
            var aRegion = new Region(myRect);
        ...
        private bool bCalcTailPos(Point ptA, bool bOnlyBelow)
        {
            ...
            else
            {
                ...
                tailSide = sdSide.sd_top;
            }

            // look for best horizontal fit
            var nDistToEdge = nSWd - ptA.X;
        ...
        public void setBalloonPosition(Form onForm, Control onControl)
        {
            ...
            var ptScreen = onForm.PointToScreen(ptA);
        ...
        public void setBalloonPosition(Form onForm, Point atPoint)
        {
            ...
            var ptScreen = onForm.PointToScreen(m_rtAnchor.Location);
        ...
        public void UpdateBalloonPosition(Point atPoint)
        {
            // calculate absolute position of the point
            m_rtAnchor = new Rectangle(atPoint, new Size(0, 0));
            var ptScreen = m_fmParent.PointToScreen(m_rtAnchor.Location);
        ...
        public void Parent_Move(object sender, EventArgs e)
        {
            ...
            var ptHigh = m_rtAnchor.Location;
            var ptLow = ptHigh;
            ptLow.Y += m_rtAnchor.Height;
            var ptScreen = ((Form)sender).PointToScreen(ptLow);
        ...
        private bool bMakeShadow()
        {
            ...
            if (m_fmShadow == null)
            {
                m_fmShadow = new CBalloonBase();
                Owner = m_fmShadow;
                m_fmShadow.Width = Width;
                m_fmShadow.Height = Height;
                m_fmShadow.StartPosition = FormStartPosition.Manual;
                m_fmShadow.Location = new Point(Location.X + C_SHADOW, Location.Y + C_SHADOW);
        ...
        //}
        private void CBalloonBase_Paint(object sender, PaintEventArgs e)
        {
            reDrawMe();
        }

        private void CBalloonBase_Click(object sender, EventArgs e)
        {
        ...
        private void CBalloonBase_Resize(object sender, EventArgs e)
        {
            reSizeMe();
        }
        ...
        private void CBalloonBase_VisibleChanged(object sender, EventArgs e)
        {
            if (!m_bHasShadow) return;

            if (Visible && m_fmShadow != null)
                m_fmShadow.Show();
            //add by lulianqi when hide the base window the m_fmShadow is not hide
            else if (Visible == false && m_fmShadow != null) m_fmShadow.Visible = false;
        }

        private void CBalloonBase_Closing(object sender, CancelEventArgs e)
        {
        ...
        private void CBalloonBase_Move(object sender, EventArgs e)
        {
            if (m_fmShadow != null) m_fmShadow.Location = new Point(Location.X + C_SHADOW, Location.Y + C_SHADOW);
        }
        ...
        public sdSide tailSide
        {
            set
            {
                m_tipSide = value;
                reSizeMe();
                if (m_bHasShadow && m_fmShadow != null) m_fmShadow.tailSide = value;
            }
        ...
        public int tailOffset
        {
            set
            {
                m_nTailOffset = value;
                reSizeMe();
                if (m_fmShadow != null) m_fmShadow.tailOffset = value;
            }
        ...
        public alAlign tailAlign
        {
            set
            {
                ...
                reSizeMe();
                if (m_fmShadow != null) m_fmShadow.tailAlign = value;
            }
            get => m_tipAlign;
        }
