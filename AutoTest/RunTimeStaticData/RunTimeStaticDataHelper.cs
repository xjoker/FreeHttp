...
namespace FreeHttp.AutoTest.RunTimeStaticData
{
    ...
    {
        public static bool AddStaticDataToCollection(ActuatorStaticDataCollection actuatorStaticDataCollection,
            CaseStaticDataType dataType, string staticDataKey, string staticDataVaule, out string errorMes)
        {
            errorMes = null;
            ...
            if (actuatorStaticDataCollection.IsHaveSameKey(staticDataKey))
            {
                errorMes = string.Format(
                    "find same key 【{0}】in RunTimeParameter with [ CaseStaticDataClass.caseStaticDataKey] in - ScriptRunTime ,and will drop this key",
                    staticDataKey);
                return false;
            }
            ...
            switch (CaseRunTimeDataTypeEngine.dictionaryStaticDataTypeClass[dataType])
            {
                case CaseStaticDataClass.caseStaticDataKey:
                    ...
                    {
                        if (!actuatorStaticDataCollection.AddStaticDataKey(staticDataKey,
                                new MyStaticDataValue(staticDataVaule)))
                        {
                            errorMes = string.Format("can not add {0} into ActuatorStaticDataCollection",
                                staticDataKey);
                            return false;
                        }
                    ...
                    else
                    {
                        throw new Exception(string.Format(
                            "find nonsupport Protocol 【{0}】with [ CaseStaticDataClass.caseStaticDataKey] in - ScriptRunTime ",
                            dataType));
                    }
                ...
                case CaseStaticDataClass.caseStaticDataParameter:
                    ...
                    if (CaseRunTimeDataTypeEngine.dictionaryStaticDataParameterAction[dataType](
                            out tempRunTimeStaticData, out tempTypeError, staticDataVaule))
                    {
                    ...
                    {
                        errorMes = string.Format("find error in 【RunTimeStaticData】->【{0}】:value:【{1}】 by {2}",
                            staticDataKey, staticDataVaule, tempTypeError);
                        return false;
                    }
                ...
                case CaseStaticDataClass.caseStaticDataSource:
                    ...
                    if (CaseRunTimeDataTypeEngine.dictionaryStaticDataSourceAction[dataType](out tempRunTimeDataSource,
