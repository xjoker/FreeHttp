...
namespace FreeHttp.AutoTest.MyCommonHelper.FileHelper
{
    ...
    public sealed class CsvFileHelper : IDisposable
    {
        ...
        public Type DataSouceType { get; } = Type.File;
        ...
        private List<string> ParseLine(string line)
        {
            ...
            var inColumn = false; //是否是在一个列元素里
            var inQuotes = false; //是否需要转义
            var isNotEnd = false; //读取完毕未结束转义
            ...
            for (var i = 0; i < line.Length; i++)
            {
                ...
                else if (character == CsvSeparator)
                {
                    inColumn = false;
                }
            ...
            if (inColumn)
            {
                if (isNotEnd) _columnBuilder.Append("\r\n");
                Fields.Add(TrimColumns ? _columnBuilder.ToString().Trim() : _columnBuilder.ToString());
            }
        ...
        private List<string> ParseContinueLine(string line)
        {
            ...
            for (var i = 0; i < line.Length; i++)
            {
                ...
                if (character == '"' && line[i + 1] == CsvSeparator) //结束转义，且后面有可能还有数据
                {
                    Fields.Add(TrimColumns ? _columnBuilder.ToString().TrimEnd() : _columnBuilder.ToString());
                    i++; //跳过下一个字符
        ...
        {
            _stream.Position = 0;
            var tempListCsvData = new List<List<string>>();
            var isNotEndLine = false;
            //这里的ReadLine可能把转义的/r/n分割，需要后面单独处理
            var tempCsvRowString = _streamReader.ReadLine();
            while (tempCsvRowString != null)
            {
                ...
                if (isNotEndLine)
                {
                    ...
                    isNotEndLine = tempCsvRowList.Count > 0 &&
                                   tempCsvRowList[tempCsvRowList.Count - 1].EndsWith("\r\n");
                    var myNowContinueList = tempListCsvData[tempListCsvData.Count - 1];
        ...
        */
        ...
        {
            foreach (var tempField in yourListCsvData)
            {
                if (tempField == null || tempField.Count == 0) continue;
        ...
        private static void WriteCsvLine(List<string> fields, TextWriter writer)
        {
            ...
            var myStrBld = new StringBuilder();
            ...
            for (var i = 0; i < fields.Count; i++)
            {
                ...
                {
                    var quotesRequired = IsSaveAsExcel
                        ? fields[i].Contains(DefaultCsvSeparator) || fields[i].Contains("\r\n") ||
                          fields[i].Contains("\"")
                        : fields[i].Contains(DefaultCsvSeparator) || fields[i].Contains("\r\n") ||
                          fields[i].StartsWith("\"");
                    if (quotesRequired)
                ...
                if (i < fields.Count - 1) myStrBld.Append(DefaultCsvSeparator);
            }

            writer.WriteLine(myStrBld.ToString());
        }

        public static void SaveCsvFile(string yourFilePath, List<List<string>> yourDataSouse, bool isAppend,
            Encoding yourEncode)
        {
            //FileStream myCsvStream = new FileStream(yourFilePath, FileMode.Create, FileAccess.ReadWrite);
            if (isAppend && !File.Exists(yourFilePath)) throw new Exception("in Append mode the FilePath must exist");
            ...
            var myCsvSw =
                new StreamWriter(
                    new FileStream(yourFilePath, isAppend ? FileMode.Append : FileMode.Create, FileAccess.Write,
                        FileShare.ReadWrite), yourEncode);
            if (yourDataSouse == null) throw new Exception("your DataSouse is null");
