...
namespace FreeHttp.AutoTest
{
    public static class MyExtensionMethods
    {
        ...
        public static bool MySplitToIntArray(this string str, char yourSplitChar, out int[] yourIntArray)
        {
            ...
            var strArray = str.Split(new[] { yourSplitChar }, StringSplitOptions.None);
        ...
        public static bool MySplitIntEnd(this string str, char yourSplitChar, out string yourStr, out int yourInt)
        {
            ...
            {
                var lastSplitCharIndex = str.LastIndexOf(yourSplitChar);
                if (lastSplitCharIndex == str.Length - 1) // 如果使用endwith会产生多余的string对象
                    return false;
                if (int.TryParse(str.Substring(lastSplitCharIndex + 1, str.Length - lastSplitCharIndex - 1),
                        out yourInt))
                {
        ...
        {
            if (nvc != null) nvc.Add(yourName, yourValue);
        }
        ...
        public static string MyToFormatString(this NameValueCollection nvc)
        {
            if (nvc != null)
                if (nvc.Count > 0)
                {
                    ...
                    var tempStrBld = new StringBuilder("{ ", nvc.Count * 32);
                    foreach (string tempKey in nvc.Keys) tempStrBld.AppendFormat("[{0}:{1}] ", tempKey, nvc[tempKey]);
                    tempStrBld.Append("}");
        ...
        {
            var cloneDc = new Dictionary<TKey, TValue>();
            foreach (var tempKvp in dc) cloneDc.Add(tempKvp.Key, tempKvp.Value);
            return cloneDc;
        }
        ...
        /// <param name="dc">目标Dictionary</param>
        /// <returns>对象的深度克隆</returns>
        public static Dictionary<TKey, TValue> MyDeepClone<TKey, TValue>(this Dictionary<TKey, TValue> dc)
            where TValue : ICloneable
        {
            var cloneDc = new Dictionary<TKey, TValue>();
            foreach (var tempKvp in dc) cloneDc.Add(tempKvp.Key, (TValue)tempKvp.Value.Clone());
            return cloneDc;
        }

        public static Dictionary<string, TTargetValue> ToChangeType<TValue, TTargetValue>(
            this Dictionary<string, TValue> dc) where TValue : TTargetValue
        {
            if (dc != null)
            {
                var newDc = new Dictionary<string, TTargetValue>();
                foreach (var item in dc) newDc.Add(item.Key, item.Value);
                return newDc;
            }
        ...
        public static void Add(this List<List<string>> myList, object[] yourValue)
        {
            if (yourValue != null)
            {
                var tempAddList = new List<string>(yourValue.Length);
