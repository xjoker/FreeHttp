...
namespace FreeHttp.AutoTest.ParameterizationPick
{
    public class ParameterPickHelper
    {
        ...
        public static string[] PickXmlParameter(string yourTarget, string yourSouce)
        {
            ...
            {
                var backStrs = new string[tempNodeList.Count];
                for (var i = 0; i < tempNodeList.Count; i++) backStrs[i] = tempNodeList[i].InnerXml;
                return backStrs;
            }
        ...
        public static string[] PickRegexParameter(string yourTarget, string yourSouce)
        {
            ...
            if (mc.Count > 0)
            {
                var backStrs = new string[mc.Count];
                for (var i = 0; i < mc.Count; i++) backStrs[i] = mc[i].Value;
                return backStrs;
            }
        ...
        {
            if (yourStrStart < 0 || yourStrLen < 0) return null;
            if (yourSouce.Length >= yourStrStart + yourStrLen)
            {
                if (yourStrLen == 0) return yourSouce.Remove(0, yourStrStart);
        ...
        {
            if (yourStrLen < 0) return null;
            if (yourSouce.Contains(yourTarget))
            {
                string tempPickStr;
                var tempStart = yourSouce.IndexOf(yourTarget) + yourTarget.Length;
        ...
        public static string PickStrParameter(string yourTarget, string yourStrEnd, string yourSouce)
        {
            if (yourSouce.Contains(yourTarget))
            {
                ...
                {
                    var tempEnd = tempPickStr.IndexOf(yourStrEnd);
        ...
        {
            if (string.IsNullOrEmpty(sourceStr) || string.IsNullOrEmpty(pickExpression) ||
                string.IsNullOrEmpty(pickExpressionAdditional))
            ...
            switch (pickExpressionAdditional)
            {
                ...
                case "str-len":
                    if (!int.TryParse(backStr, out strLen))
                        throw new Exception(string.Format("this Expressions error :{0}",
                            "the len should be a int value"));
                ...
                case "index-len":
                    ...
                    if (!int.TryParse(frontStr, out frontIndex))
                        throw new Exception(string.Format("this Expressions error :{0}",
                            "the index should be a int value"));
        ...
        public static string ParameterPickXml(string sourceStr, string pickExpression, string pickExpressionAdditional)
        {
            ...
            if (returnIndex < 0) throw new Exception("this PickTypeAdditional should greater than 0");
            var returnArray = PickXmlParameter(pickExpressionAdditional, sourceStr);
            if (returnArray == null) return null;
            if (returnIndex == 0)
                return string.Join(",", returnArray);
            if (returnIndex > returnArray.Length)
                return null;
            return returnArray[returnIndex - 1];
        }

        public static string ParameterPickRegex(string sourceStr, string pickExpression,
            string pickExpressionAdditional)
        {
            {
                if (string.IsNullOrEmpty(sourceStr) || string.IsNullOrEmpty(pickExpression) ||
                    string.IsNullOrEmpty(pickExpressionAdditional))
                ...
                if (!int.TryParse(pickExpressionAdditional, out returnIndex))
                    throw new Exception("this PickTypeAdditional should be a number");
                if (returnIndex < 0) throw new Exception("this PickTypeAdditional should greater than 0");
                var returnArray = PickRegexParameter(pickExpression, sourceStr);
                if (returnArray == null) return null;
                if (returnIndex == 0)
                    return string.Join(",", returnArray);
                if (returnIndex > returnArray.Length)
        ...
        {
            if (string.IsNullOrEmpty(yourParameterPick.PickTypeExpression)) return "your Expression is empty";
            switch (yourParameterPick.PickType)
            {
                case ParameterPickType.Regex:
                    ...
                    {
                        if (tempAdditionalIndex < 0) return "this PickTypeAdditional should greater than 0";
                    }
                    else
                    {
                    ...
                    try
                    {
                        Regex.Matches("", yourParameterPick.PickTypeExpression);
                    }
                    catch (Exception ex)
                    {
