...
namespace FreeHttp.AutoTest
{
    ...
    public class MyBytes
    {
        ...
        {
            if (yourBytes == null) return null;
            var tempLen = yourBytes.Length;
            for (var i = yourBytes.Length - 1; i > 0; i--)
                if (yourBytes[i] == 0x00)
                    tempLen--;
                else
                    break;
            var tempBytes = new byte[tempLen];
            for (var i = 0; i < tempLen; i++) tempBytes[i] = yourBytes[i];
            return tempBytes;
        }
        ...
        {
            if (yourData == null) return "null data";
            ...
            var tempIp = "";
            for (var i = 0; i < yourData.Length; i++) tempIp += (int)yourData[i] + ".";
        ...
        public static int GetByteLen(byte[] yourLen)
        {
            ...
            var tempLen = 0;
            for (var i = 0; i < yourLen.Length; i++) tempLen += (int)Math.Pow(256, yourLen.Length - i - 1) * yourLen[i];
            return tempLen;
        }
        ...
        {
            var tempData = BitConverter.GetBytes(yourLen);
            var dataToBack = new byte[2];
        ...
        {
            if (yourByteList == null) throw new Exception("yourByteList is null");
            var byteLen = 0;
            var nowCopyIndex = 0;
            byte[] outBytes;
            foreach (var tempBytes in yourByteList) byteLen += tempBytes.Length;
            outBytes = new byte[byteLen];
            foreach (var tempBytes in yourByteList)
            {
        ...
        public static byte[] ReplaceBytes(byte[] src, byte[] search, byte[] repl)
        {
            ...
            Buffer.BlockCopy(src, index + search.Length, dst, index + repl.Length,
        ...
        {
            if (src == null || find == null || src.Length == 0 || find.Length == 0 ||
                find.Length > src.Length) return -1;
            for (var i = 0; i < src.Length - find.Length + 1; i++)
                if (src[i] == find[0])
                {
                    if (find.Length == 1) return i;
                    for (var m = 1; m < find.Length; m++)
                    {
        ...
        private static readonly Dictionary<HexDecimal, int> DictionaryHexDecimal = new Dictionary<HexDecimal, int>
            { { HexDecimal.hex2, 8 }, { HexDecimal.hex10, 3 }, { HexDecimal.hex16, 2 } };

        private static readonly Dictionary<ShowHexMode, string> DictionaryShowHexMode =
            new Dictionary<ShowHexMode, string>
            {
                { ShowHexMode.spitSpace0x, " 0x" }, { ShowHexMode.spit0x, "0x" }, { ShowHexMode.spitSpace0d, " 0d" },
                { ShowHexMode.spit0d, "0d" }, { ShowHexMode.spitSpace0b, " 0b" }, { ShowHexMode.spit0b, "0b" },
                { ShowHexMode.spitM_, "-" }, { ShowHexMode.spit_, "_" }, { ShowHexMode.space, " " },
                { ShowHexMode.@null, "" }
            };
        ...
        /// <returns>返回结果</returns>
        public static string StringToHexString(string yourStr, Encoding encode, HexDecimal hexaDecimal,
            ShowHexMode stringMode)
        {
            var tempBytes = encode.GetBytes(yourStr);
        ...
        public static string ByteToHexString(byte[] yourBytes, HexDecimal hexDecimal, ShowHexMode stringMode)
        {
            ...
            if (yourBytes == null) return null;
            var result = new StringBuilder(DictionaryHexDecimal[hexDecimal] + DictionaryShowHexMode[stringMode].Length);

            for (var i = 0; i < yourBytes.Length; i++)
            {
                result.Append(DictionaryShowHexMode[stringMode]);
                result.Append(Convert.ToString(yourBytes[i], (int)hexDecimal)
                    .PadLeft(DictionaryHexDecimal[hexDecimal], '0'));
            }
        ...
        {
            if (hexDecimal == HexDecimal.hex16 && (stringMode == ShowHexMode.spit0b ||
                                                   stringMode == ShowHexMode.spit0d ||
                                                   stringMode == ShowHexMode.spitSpace0b ||
                                                   stringMode == ShowHexMode.spitSpace0d))
                throw new Exception("your HexaDecimal and ShowHexMode is conflict");
            string[] hexStrs;
            byte[] resultBytes;
            var modeStr = string.Empty; //string.Empty 不等于 null
            if (stringMode != ShowHexMode.@null) modeStr = DictionaryShowHexMode[stringMode];
            if (modeStr == string.Empty)
            {
                ...
                for (var startIndex = 0; startIndex < tempHexNum; startIndex++)
                    hexStrs[startIndex] = yourStr.Substring(startIndex * DictionaryHexDecimal[hexDecimal],
                        DictionaryHexDecimal[hexDecimal]);
            }
            else
            {
                ...
                hexStrs = yourStr.Split(new[] { modeStr }, StringSplitOptions.RemoveEmptyEntries);
            }

            try
            {
                resultBytes = new byte[hexStrs.Length];
                for (var i = 0; i < hexStrs.Length; i++) resultBytes[i] = Convert.ToByte(hexStrs[i], (int)hexDecimal);
            }
            catch (Exception ex)
            {
                throw new Exception(
                    string.Format(
                        "error data ,can not change your hex string to your hex,  hexDecimal[{0}] ShowHexMode[{1}]",
                        hexDecimal, stringMode), ex);
            }
        ...
        public static byte[] HexStringToByte(string yourStr, HexDecimal hexDecimal)
        {
            ...
            {
                if (hexDecimal == HexDecimal.hex16 && (tryStringSpitMode.Key == ShowHexMode.spit0b ||
                                                       tryStringSpitMode.Key == ShowHexMode.spit0d ||
                                                       tryStringSpitMode.Key == ShowHexMode.spitSpace0b ||
                                                       tryStringSpitMode.Key == ShowHexMode.spitSpace0d)) continue;
